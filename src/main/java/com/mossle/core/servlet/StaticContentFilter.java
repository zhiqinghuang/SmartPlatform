...
public class StaticContentFilter implements Filter {
	/** 需要被Gzip压缩的Mime类型. */
	public static final String[] GZIP_MIME_TYPES = { "text/html", "application/xhtml+xml", "text/plain", "text/css", "text/javascript", "application/x-javascript", "application/json" };
	...
	public void doFilter(ServletRequest req, ServletResponse res, FilterChain filterChain) throws ServletException, IOException {
		...
		String contentPath = requestUri.substring(request.getContextPath().length());
		...
		if (!ServletUtils.checkIfModifiedSince(request, response, contentInfo.getLastModified()) || !ServletUtils.checkIfNoneMatchEtag(request, response, contentInfo.getEtag())) {
			return;
		}
		// 设置Etag/过期时间
	...
	private ContentInfo getContentInfo(String contentPath) {
		...
		String realFilePath = filterConfig.getServletContext().getRealPath(contentPath);
