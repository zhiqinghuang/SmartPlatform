...
public class HibernateGenericDao extends HibernateBasicDao {
	...
	public <T> List<T> findByLike(Class<T> entityClass, String name, Object value) {
		...
		return this.createCriteria(entityClass, Restrictions.like(name, value)).list();
	}
	...
	public <T> T findUniqueBy(Class<T> entityClass, String name, Object value) {
		return (T) this.createCriteria(entityClass, Restrictions.eq(name, value)).setMaxResults(1).uniqueResult();
	}
	 ...
	 */
	@Transactional(readOnly = true)
	public <T> boolean isUnique(Class<T> entityClass, T entity, String uniquePropertyNames) throws NoSuchMethodException, IllegalAccessException, InvocationTargetException {
		...
		for (String name : nameList) {
			String getterMethodName = ReflectUtils.getGetterMethodName(entity, name);
			criteria.add(Restrictions.eq(name, BeanUtils.invokeMethod(entity, getterMethodName)));
		}
