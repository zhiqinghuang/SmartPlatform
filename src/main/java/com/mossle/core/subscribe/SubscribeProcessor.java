...
public class SubscribeProcessor implements ApplicationContextAware {
	...
	public void buildComposite() throws Exception {
		...
		for (Map.Entry<String, String> entry : map.entrySet()) {
			...
			Subscribable subscribable = (Subscribable) applicationContext.getBean(Class.forName(subscribeClassName));
			logger.info("subscribable : " + subscribable);
			CompositeSubscriber compositeSubscriber = compositeMap.get(subscribeName);
			...
			defaultMessageListenerContainer.setPubSubDomain(subscribable.isTopic());
			defaultMessageListenerContainer.setConnectionFactory(connectionFactory);
			defaultMessageListenerContainer.setDestinationName(subscribable.getName());
