...
/**
 * 对传入的URL附加key，计算md5，得到一个32位的字符串。 MD5的值确实是16进制的，所以它可以表示16 ^ 32 =
 * 3.4028236692093846346337460743177e+38种组合 把md5分成四段，每段是16 ^ 8 = 4 294 967
 * 296，40多亿，认为这种的碰撞几率也是很小的 然后用a-zA-Z0-9表示6位的62进制，62 ^ 6 = 56 800 235
 * 584，500多亿，可以把上面的1/4映射到里边
 * 
 * 但是此处的代码和http://www.jb51.net/article/29308.htm描述的有差别。 体现在三点
 * 网址描述的是a-z0-5共32个字符。所以总数是 32 ^ 6 = 1 073 741 824，10多亿，比40亿小
 * 所以需要先二进制与0x3FFFFFFF，只处理32位，否则就超出了，不过32位也比32 ^ 6多一，不过几率很小，可以忽略。
 * 然后就是最后计算每次二进制右移5，2 ^ 5 = 32也是按照32位来计算的，如果按照62位计算，应该除以62才对。
 */
