...
public class HttpConnectionPool implements Runnable {
	...
	public String process(String method, Map<String, String> queryParams, Map<String, String> formParams, Map<String, String> headParams) {
		boolean success = false;
		HttpConnectionResult httpConnectionResult = null;

		while (!success) {
			HttpConnectionInfo httpConnectionInfo = this.tryToGetHttpConnectionInfo();
			httpConnectionResult = httpConnectionInfo.process(method, queryParams, formParams, headParams);
	...
	private HttpConnectionInfo tryToGetHttpConnectionInfo() {
		...
		try {
			if (activeHttpConnectionInfos.isEmpty()) {
				logger.error("suspended HttpConnectionInfos size is {}", suspendedHttpConnectionInfos.size());
				throw new IllegalStateException("active HttpConnectionInfos is empty");
			}

			int temperaryIndex = index.incrementAndGet() % activeHttpConnectionInfos.size();
	...
	public void run() {
		while (running) {
			...
			List<HttpConnectionInfo> checkedHttpConnectionInfos = new ArrayList(suspendedHttpConnectionInfos);
