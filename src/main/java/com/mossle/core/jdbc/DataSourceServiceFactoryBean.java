...
public class DataSourceServiceFactoryBean implements FactoryBean, InitializingBean, DisposableBean {
	 ...
	 * 构建过程.
	 * 
	 * 1.创建DataSourceService 2.通过properties设置DataSourceService初始化参数
	 * 3.通过properties创建每个DataSourceInfo
	 * 4.把DataSourceInfo注册到dataSourceService中，并进行初始化
	 */
	public void afterPropertiesSet() {
		dataSourceService = new DataSourceService();
		dataSourceService.setLog4jdbcEnabled(this.checkLog4jdbcEnabled());
		Collection<DataSourceInfo> dataSourceInfos = new DbcpDataSourceInfoBuilder(defaultPrefix, properties).build();
	...
	public void destroy() {
		...
		for (DataSourceWrapper dataSourceWrapper : dataSourceService.getDataSources()) {
		...
		for (DataSourceInfo dataSourceInfo : dataSourceService.getDataSourceInfos()) {
			...
			for (Method method : dataSourceInfo.getClass().getDeclaredMethods()) {
				...
				if (methodName.startsWith("get") || methodName.startsWith("is")) {
					...
					buff.append(prefix).append(fieldName).append("=").append(fieldValue).append("\n");
				}
			}
		}
