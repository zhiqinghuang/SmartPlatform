...
public class TaskDeadlineJob {
	private static Logger logger = LoggerFactory.getLogger(TaskDeadlineJob.class);
	...
	public void execute() throws Exception {
		...
		for (TaskDeadline taskDeadline : taskDeadlines) {
			if (now.after(taskDeadline.getDeadlineTime())) {
				...
				String templateCode = taskDeadline.getNotificationTemplateCode();

				this.doNotice(taskDeadline.getTaskInfo(), type, receiver, templateCode);
			}
		}
	...
	public void doNotice(TaskInfo taskInfo, String type, String receiver, String templateCode) {
		...
		if ("任务接收人".equals(receiver)) {
			...
			String initiator = internalProcessConnector.findInitiator(taskInfo.getProcessInstanceId());
