...
public class DatabaseKeyValueConnector implements KeyValueConnector {
	...
	public List<Record> findByStatus(int status, String userId, String tenantId) {
		List<Map<String, Object>> list = jdbcTemplate.queryForList("SELECT * FROM KV_RECORD WHERE STATUS=? AND USER_ID=? AND TENANT_ID=?", status, userId, tenantId);
	...
	public Record convertRecord(Map<String, Object> recordMap) {
		...
		List<Map<String, Object>> list = jdbcTemplate.queryForList("select * from KV_PROP where record_id=?", record.getCode());
	...
	public void insert(Record record) {
		String sqlRecordInsert = "insert into KV_RECORD(name,form_template_code,category,status,ref,create_time,user_id,tenant_id)" + " values(?,?,?,?,?,?,?,?)";
		...
		jdbcTemplate.update(sqlRecordInsert, name, formTemplateCode, record.getCategory(), record.getStatus(), ref, createTime, userId, tenantId);
		...
		for (Prop prop : record.getProps().values()) {
			jdbcTemplate.update(sqlProp, prop.getCode(), prop.getType(), prop.getValue(), record.getCode());
		}
	}
	...
	public void update(Record record) {
		...
		jdbcTemplate.update(sqlRecord, record.getName(), record.getFormTemplateCode(), record.getCategory(), record.getStatus(), record.getRef(), record.getCode());
		...
		for (Prop prop : record.getProps().values()) {
			...
			if (resultRecord.getProps().containsKey(prop.getCode())) {
				...
				jdbcTemplate.update(sqlPropInsert, prop.getCode(), prop.getType(), prop.getValue(), record.getCode());
			}
		}
	}
	...
	public long findTotalCount(String category, String tenantId, List<PropertyFilter> propertyFilters) {
		...
		if (propertyFilters.isEmpty()) {
			...
			for (PropertyFilter propertyFilter : propertyFilters) {
				...
				sqlPrefix += (" join KV_PROP " + propName + " on r.ID=" + propName + ".RECORD_ID and " + propName + ".CODE=? and " + propName + ".VALUE like ?");
	...
	public List<Map<String, Object>> findResult(Page page, String category, String tenantId, Map<String, String> headers, List<PropertyFilter> propertyFilters) {
		String sqlPrefix = null;
		List<Object> params = new ArrayList<Object>();
		...
		if (page.isOrderEnabled()) {
			...
			if (usedFieldMap.containsKey(orderBy)) {
				...
				String propName = "p";
				sqlPrefix += (" join KV_PROP " + propName + " on r.ID=" + propName + ".RECORD_ID and " + propName + ".CODE='" + orderBy + "'");
				sqlOrder = " order by " + propName + ".VALUE " + order;
			}
		}
		...
		for (Map<String, Object> record : records) {
			...
			List<Map<String, Object>> props = jdbcTemplate.queryForList("select * from KV_PROP where RECORD_ID=?", recordId);
