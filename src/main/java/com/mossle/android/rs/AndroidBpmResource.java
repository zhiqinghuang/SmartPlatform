...
public class AndroidBpmResource {
	private static Logger logger = LoggerFactory.getLogger(AndroidBpmResource.class);
	...
	public BaseDTO processDefinitions() throws Exception {
		...
		for (BpmProcess bpmProcess : bpmProcesses) {
			...
			map.put("processDefinitionId", bpmProcess.getBpmConfBase().getProcessDefinitionId());
			list.add(map);
		}
	...
	@Produces(MediaType.APPLICATION_JSON)
	public BaseDTO processInstances(@HeaderParam("sessionId") String sessionId) throws Exception {
		logger.info("start");
	...
	@Path("startProcess")
	@Produces(MediaType.APPLICATION_JSON)
	public BaseDTO startProcess(@HeaderParam("sessionId") String sessionId, @FormParam("processDefinitionId") String processDefinitionId, @FormParam("data") String data) throws Exception {
		...
		BpmProcess bpmProcess = bpmProcessManager.findUnique(hql, processDefinitionId);
		...
		String processInstanceId = processConnector.startProcess(userId, formParameter.getBusinessKey(), processDefinitionId, processParameters);
