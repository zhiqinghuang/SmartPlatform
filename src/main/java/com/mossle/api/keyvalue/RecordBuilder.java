...
public class RecordBuilder {
	...
	public Record build(Record record, int status, FormParameter formParameter) {
		...
		for (Map.Entry<String, List<String>> entry : formParameter.getMultiValueMap().entrySet()) {
	...
	public Record build(Record record, MultipartHandler multipartHandler, StoreConnector storeConnector, String tenantId) throws Exception {
		...
		for (Map.Entry<String, List<MultipartFile>> entry : multipartHandler.getMultiFileMap().entrySet()) {
			...
			prop.setCode(key);
			prop.setType(0);
			prop.setValue(storeConnector.saveStore("form", new MultipartFileDataSource(multipartFile), tenantId).getKey());
			record.getProps().put(prop.getCode(), prop);
		}
		return record;
	}
