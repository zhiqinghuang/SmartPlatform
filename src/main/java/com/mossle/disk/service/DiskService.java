...
public class DiskService {
	 ...
	 * 上传文件.
	 */
	public DiskInfo createFile(String userId, DataSource dataSource, String name, long size, String parentPath, String tenantId) throws Exception {
		StoreDTO storeDto = storeConnector.saveStore("default/user/" + userId, dataSource, tenantId);
		String type = FileUtils.getSuffix(name);

		return this.createDiskInfo(userId, name, size, storeDto.getKey(), type, 1, parentPath);
	}
	 ...
	 * 上传文件，或新建文件夹.
	 */
	public DiskInfo createDiskInfo(String userId, String name, long size, String ref, String type, int dirType, String parentPath) {
		if (name == null) {
	...
	public String move(String userId, Long id, Long parentId) {
		...
		if (parentId != 0) {
			...
			if (!"dir".equals(parent.getType())) {
				logger.info("{}({}) is not directory", parent.getParentPath() + "/" + parent.getName(), parentId);
			...
			if ("dir".equals(diskInfo.getType()) && checkedParentPath.startsWith(currentPath)) {
				logger.info("{}({}) is sub directory of {}({})", parent.getParentPath() + "/" + parent.getName(), parentId, diskInfo.getParentPath() + "/" + diskInfo.getName(), id);
