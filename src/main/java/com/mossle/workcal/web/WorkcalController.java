...
public class WorkcalController {
	private static Logger logger = LoggerFactory.getLogger(WorkcalController.class);
	...
	public String list(@RequestParam(value = "year", required = false) Integer year, Model model) {
		...
		// 每周的工作规则
		List<WorkcalRule> workcalRules = workcalRuleManager.find("from WorkcalRule where year=? and status=? and tenantId=?", year, STATUS_WEEK, tenantId);
		...
		// 特殊日期
		List<WorkcalRule> extraWorkcalRules = workcalRuleManager.find("from WorkcalRule where year=? and status<>?", year, STATUS_WEEK);
