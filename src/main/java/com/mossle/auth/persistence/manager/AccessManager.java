...
public class AccessManager extends HibernateEntityDao<Access> {
	...
	public void batchSave(String tenantId, String type, List<String> ids, List<String> values, List<String> perms) {
		int priority = 0;
		...
		for (int i = 0; i < values.size(); i++) {
			...
			Access access = createOrGetAccess(idStr, value, type, permStr, tenantId);
	...
	public Access createOrGetAccess(String id, String value, String type, String perm, String tenantId) {
		Access access;

		if (id.length() != 0) {
			access = this.get(Long.parseLong(id));
		} else {
			access = this.findUnique("from Access where value=? and type=? and tenant_id=?", value, type, tenantId);
		}
	...
	public Perm createOrGetPerm(String code, String tenantId) {
		Perm perm = this.findUnique("from Perm where code=? and tenant_id=?", code, tenantId);
