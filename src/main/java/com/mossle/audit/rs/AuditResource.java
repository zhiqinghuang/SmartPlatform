...
public class AuditResource {
	...
	@POST
	public BaseDTO log(@FormParam("user") String user, @FormParam("resourceType") String resourceType, @FormParam("resourceId") String resourceId, @FormParam("action") String action, @FormParam("result") String result, @FormParam("application") String application, @FormParam("auditTime") String auditTime, @FormParam("client") String client, @FormParam("server") String server, @FormParam("description") String description) {
		Date date = new Date();

		try {
			date = new SimpleDateFormat("yyyy-MM-dd'T'HH:mm:ss.S").parse(auditTime);
		} catch (Exception ex) {
