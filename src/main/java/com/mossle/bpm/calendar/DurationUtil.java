...
public class DurationUtil {
	...
	public DurationUtil(String text, AdvancedBusinessCalendar businessCalendar) throws Exception {
		...
		if (expressions.get(0).startsWith("R")) {
			this.isRepeat = true;
			this.times = (expressions.get(0).length() == 1) ? Integer.MAX_VALUE : Integer.parseInt(expressions.get(0).substring(1));
		...
		if (this.isDuration(expressions.get(0))) {
			...
			// 如果有后半部分，就是结束时间
			// 如果没有，就可能是无限循环
			this.end = (expressions.size() == 1) ? null : this.parseDate(expressions.get(1));
		} else {
			// 如果不是P开头，就是开始时间
