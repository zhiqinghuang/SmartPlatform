...
public class ActivitiHistoryGraphBuilder {
	...
	public void fetchProcessDefinitionEntity() {
		String processDefinitionId = Context.getCommandContext().getHistoricProcessInstanceEntityManager().findHistoricProcessInstance(processInstanceId).getProcessDefinitionId();
	...
	public void fetchHistoricActivityInstances() {
		...
		historicActivityInstances = Context.getCommandContext().getHistoricActivityInstanceEntityManager().findHistoricActivityInstancesByQueryCriteria(historicActivityInstanceQueryImpl, page);
	}
	...
	public Edge findPreviousEdge(Node currentNode, long currentStartTime) {
		...
		for (PvmTransition pvmTransition : activityImpl.getIncomingTransitions()) {
			...
			} else if ((currentStartTime - nestestHistoricActivityInstance.getEndTime().getTime()) > (currentStartTime - visitiedHistoryActivityInstance.getEndTime().getTime())) {
				// 寻找离当前节点最近的上一个节点
