...
public class ConsoleController {
	...
	public String removeDeployment(@RequestParam("deploymentId") String deploymentId) {
		...
		for (ProcessDefinition processDefinition : processDefinitions) {
			...
			List<BpmConfBase> bpmConfBases = bpmConfBaseManager.find(hql, processDefinition.getId(), processDefinition.getKey(), processDefinition.getVersion());

			for (BpmConfBase bpmConfBase : bpmConfBases) {
				for (BpmConfNode bpmConfNode : bpmConfBase.getBpmConfNodes()) {
					for (BpmConfCountersign bpmConfCountersign : bpmConfNode.getBpmConfCountersigns()) {
					...
					for (BpmConfListener bpmConfListener : bpmConfNode.getBpmConfListeners()) {
					...
					for (BpmConfOperation bpmConfOperation : bpmConfNode.getBpmConfOperations()) {
	...
	public void graphProcessDefinition(@RequestParam("processDefinitionId") String processDefinitionId, HttpServletResponse response) throws Exception {
	...
	public void viewXml(@RequestParam("processDefinitionId") String processDefinitionId, HttpServletResponse response) throws Exception {
		RepositoryService repositoryService = processEngine.getRepositoryService();
	...
	public String removeProcessInstance(@RequestParam("processInstanceId") String processInstanceId, @RequestParam("deleteReason") String deleteReason) {
	...
	public String prepareJump(@RequestParam("executionId") String executionId, Model model) {
	...
	public String beforeUpdateProcess(@RequestParam("processDefinitionId") String processDefinitionId, Model model) throws Exception {
		ProcessDefinition processDefinition = processEngine.getRepositoryService().getProcessDefinition(processDefinitionId);
		InputStream is = processEngine.getRepositoryService().getResourceAsStream(processDefinition.getDeploymentId(), processDefinition.getResourceName());
	...
	public String doUpdateProcess(@RequestParam("processDefinitionId") String processDefinitionId, @RequestParam("xml") String xml) throws Exception {
	...
	public String migrateInput(@RequestParam("processInstanceId") String processInstanceId, Model model) {
		...
		model.addAttribute("processDefinitions", processEngine.getRepositoryService().createProcessDefinitionQuery().list());
	...
	public String migrateInput(@RequestParam("processInstanceId") String processInstanceId, @RequestParam("processDefinitionId") String processDefinitionId) {
		processEngine.getManagementService().executeCommand(new MigrateCmd(processInstanceId, processDefinitionId));
