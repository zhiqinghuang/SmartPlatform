...
public class BpmConfNoticeController {
	...
	public String list(@RequestParam("bpmConfNodeId") Long bpmConfNodeId, Model model) {
		...
		List<BpmConfNotice> bpmConfNotices = bpmConfNoticeManager.findBy("bpmConfNode", bpmConfNode);
	...
	public String save(@ModelAttribute BpmConfNotice bpmConfNotice, @RequestParam("bpmConfNodeId") Long bpmConfNodeId, @RequestParam("templateCode") String templateCode, @RequestParam("notificationTypes") List<String> notificationTypes) {
		...
		if (bpmConfNotice.getType() == 0) {
			...
			taskDefinitionConnector.addTaskNotification(taskDefinitionKey, processDefinitionId, taskNotification);
			...
			taskDefinitionConnector.addDeadline(taskDefinitionKey, processDefinitionId, deadline);
		}

		return "redirect:/bpm/bpm-conf-notice-list.do?bpmConfNodeId=" + bpmConfNodeId;
	}
	...
	public String remove(@RequestParam("id") Long id) {
		...
		if (bpmConfNotice.getType() == 0) {
			...
			taskNotification.setEventName("create");
			taskNotification.setTemplateCode(templateCode);
			taskDefinitionConnector.removeTaskNotification(taskDefinitionKey, processDefinitionId, taskNotification);
		} else if (bpmConfNotice.getType() == 1) {
			TaskNotificationDTO taskNotification = new TaskNotificationDTO();
			taskNotification.setEventName("complete");
			taskNotification.setTemplateCode(templateCode);
			...
			deadline.setNotificationReceiver(receiver);
			deadline.setNotificationType(notificationType);
			taskDefinitionConnector.removeDeadline(taskDefinitionKey, processDefinitionId, deadline);
		}
	...
	@Resource
	public void setBpmConfNoticeManager(BpmConfNoticeManager bpmConfNoticeManager) {
	...
	@Resource
	public void setBpmMailTemplateManager(BpmMailTemplateManager bpmMailTemplateManager) {
