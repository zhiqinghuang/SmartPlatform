...
public class BpmConfFormController {
	...
	public String list(@RequestParam("bpmConfNodeId") Long bpmConfNodeId, Model model) {
		...
		List<BpmConfForm> bpmConfForms = bpmConfFormManager.findBy("bpmConfNode", bpmConfNode);
	...
	public String save(@ModelAttribute BpmConfForm bpmConfForm, @RequestParam("bpmConfNodeId") Long bpmConfNodeId) {
		BpmConfForm dest = bpmConfFormManager.findUnique("from BpmConfForm where bpmConfNode.id=?", bpmConfNodeId);
		...
		taskDefinitionConnector.saveForm(taskDefinitionKey, processDefinitionId, form);
