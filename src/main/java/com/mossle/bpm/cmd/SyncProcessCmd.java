...
public class SyncProcessCmd implements Command<Void> {
	...
	public Void execute(CommandContext commandContext) {
		...
		BpmConfBase bpmConfBase = bpmConfBaseManager.findUnique("from BpmConfBase where processDefinitionKey=? and processDefinitionVersion=?", processDefinitionKey, processDefinitionVersion);
	...
	public void processGlobal(BpmnModel bpmnModel, int priority, BpmConfBase bpmConfBase) {
		...
		BpmConfNode bpmConfNode = bpmConfNodeManager.findUnique("from BpmConfNode where code=? and bpmConfBase=?", process.getId(), bpmConfBase);
	 ...
	 * 配置用户任务.
	 */
	public void processUserTask(Node node, BpmnModel bpmnModel, int priority, BpmConfBase bpmConfBase) {
		BpmConfNodeManager bpmConfNodeManager = getBpmConfNodeManager();
		BpmConfNode bpmConfNode = bpmConfNodeManager.findUnique("from BpmConfNode where code=? and bpmConfBase=?", node.getId(), bpmConfBase);
		...
		// 更新TaskDefinition
		TaskDefinitionConnector taskDefinitionConnector = this.getTaskDefinitionConnector();
		TaskDefinitionDTO taskDefinitionDto = new TaskDefinitionBuilder().setUserTask(userTask).setProcessDefinitionId(bpmConfBase.getProcessDefinitionId()).build();
	...
	public int processUserTaskConf(BpmConfNode bpmConfNode, String value, int type, int priority) {
		...
		BpmConfUser bpmConfUser = bpmConfUserManager.findUnique("from BpmConfUser where value=? and type=? and priority=? and status=0 and bpmConfNode=?", value, type, priority, bpmConfNode);
	 ...
	 * 配置开始事件.
	 */
	public void processStartEvent(Node node, BpmnModel bpmnModel, int priority, BpmConfBase bpmConfBase) {
		BpmConfNodeManager bpmConfNodeManager = getBpmConfNodeManager();
		BpmConfNode bpmConfNode = bpmConfNodeManager.findUnique("from BpmConfNode where code=? and bpmConfBase=?", node.getId(), bpmConfBase);
	 ...
	 * 配置结束事件.
	 */
	public void processEndEvent(Node node, BpmnModel bpmnModel, int priority, BpmConfBase bpmConfBase) {
		BpmConfNodeManager bpmConfNodeManager = getBpmConfNodeManager();
		BpmConfNode bpmConfNode = bpmConfNodeManager.findUnique("from BpmConfNode where code=? and bpmConfBase=?", node.getId(), bpmConfBase);
	...
	public void processListener(List<ActivitiListener> activitiListeners, BpmConfNode bpmConfNode) {
		...
		for (ActivitiListener activitiListener : activitiListeners) {
			...
			BpmConfListener bpmConfListener = bpmConfListenerManager.findUnique("from BpmConfListener where value=? and type=? and status=0 and bpmConfNode=?", value, type, bpmConfNode);
	...
	public void processForm(UserTask userTask, BpmConfNode bpmConfNode) {
		...
		BpmConfForm bpmConfForm = bpmConfFormManager.findUnique("from BpmConfForm where bpmConfNode=?", bpmConfNode);
