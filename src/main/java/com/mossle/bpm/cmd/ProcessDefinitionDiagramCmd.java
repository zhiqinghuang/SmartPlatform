...
public class ProcessDefinitionDiagramCmd implements Command<InputStream> {
	...
	public InputStream execute(CommandContext commandContext) {
		...
		if (deploymentId != null) {
			byte[] bytes = commandContext.getResourceEntityManager().findResourceByDeploymentIdAndResourceName(deploymentId, diagramResourceName).getBytes();
		...
		InputStream is = new DefaultProcessDiagramGenerator().generateDiagram(bpmnModel, "png", processEngineConfiguration.getActivityFontName(), processEngineConfiguration.getLabelFontName(), null);
