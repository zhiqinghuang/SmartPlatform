...
public class FindStartFormCmd implements Command<FormDTO> {
	...
	public FormDTO execute(CommandContext commandContext) {
		...
		if (processDefinitionEntity.hasStartFormKey()) {
			...
			DefaultFormHandler formHandler = (DefaultFormHandler) processDefinitionEntity.getStartFormHandler();
		...
		String initiatorVariableName = (String) processDefinitionEntity.getProperty(BpmnParse.PROPERTYNAME_INITIATOR_VARIABLE_NAME);

		if (!("${" + initiatorVariableName + "}").equals(expressionText)) {
			logger.info("the assignee of {} is not {}, just skip", taskDefinitionKey, "${" + initiatorVariableName + "}");
		...
		if (formHandler.getFormKey() == null) {
			...
			logger.info("cannot find formKey : {}, {}", processDefinitionId, taskDefinitionKey);
