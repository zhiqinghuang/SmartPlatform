...
public class UpdateProcessCmd implements Command<Void> {
	...
	public Void execute(CommandContext commandContext) {
		...
		try {
			...
			ProcessDefinitionDiagramCmd processDefinitionDiagramCmd = new ProcessDefinitionDiagramCmd(processDefinitionEntity.getId());
			...
			jdbcTemplate.update("update ACT_GE_BYTEARRAY set BYTES_=? where NAME_=? and DEPLOYMENT_ID_=?", pngBytes, diagramResourceName, deploymentId);
		} catch (Exception ex) {
