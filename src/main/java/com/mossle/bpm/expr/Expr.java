...
public class Expr {
	...
	public List<String> evaluate(String text, ExprProcessor exprProcessor) {
		...
		for (Token token : tokens) {
			if (token.isOper()) {
				...
				List<String> value = exprProcessor.process(left, right, token.getValue());
				stack.push(value);
			} else {
	 ...
	 * </p>
	 */
	public void processToken(Token token, Stack<Token> tokenStack, List<Token> output) {
		if (token.isOper()) {
	 ...
	 * 处理操作符.
	 */
	public void processOper(Token token, Stack<Token> tokenStack, List<Token> output) {
		...
		// 越靠前，索引越小，优先级越高
		if (opers.indexOf(innerToken.getValue()) <= opers.indexOf(token.getValue())) {
	 ...
	 * 把堆栈中保存的操作符都弹出来，忽略().
	 */
	public void popTokenStack(Stack<Token> tokenStack, List<Token> output, boolean breakWhenLeftBracket) {
