...
public class BpmResource {
	...
	@GET
	@Path("diagram")
	public JSONPObject diagram(@QueryParam("processDefinitionId") String processDefinitionId, @QueryParam("processInstanceId") String processInstanceId, @QueryParam("callback") String callback) {
		init();
	...
	@GET
	@Path("highlighted")
	public JSONPObject highlighted(@QueryParam("processInstanceId") String processInstanceId, @QueryParam("callback") String callback) {
		init();
		this.processInstanceId = processInstanceId;
	...
	public ObjectNode getDiagram() {
		...
		if (processInstanceId != null) {
			...
			for (ProcessInstance subProcessInstance : subProcessInstances) {
				...
				ProcessDefinitionEntity subDef = (ProcessDefinitionEntity) repositoryService.getDeployedProcessDefinition(subDefId);
	...
	private void getActivity(ActivityImpl activity, ArrayNode activityArray, ArrayNode sequenceFlowArray) {
		...
		for (Map.Entry<String, Object> entry : properties.entrySet()) {
			...
			if (prop instanceof String) {
				...
				List<TimerDeclarationImpl> timerDeclarations = (List<TimerDeclarationImpl>) properties.get(key);
				...
				List<EventSubscriptionDeclaration> eventDefinitions = (List<EventSubscriptionDeclaration>) properties.get(key);
	...
	@Path("next")
	@GET
	public List<ActivityDTO> findNextActivities(@QueryParam("processDefinitionId") String processDefinitionId, @QueryParam("activityId") String activityId) {
		FindNextActivitiesCmd cmd = new FindNextActivitiesCmd(processDefinitionId, activityId);

		return convertActivityDtos(processEngine.getManagementService().executeCommand(cmd));
	}

	@Path("previous")
	@GET
	public List<ActivityDTO> findPreviousActivities(@QueryParam("processDefinitionId") String processDefinitionId, @QueryParam("activityId") String activityId) {
		FindPreviousActivitiesCmd cmd = new FindPreviousActivitiesCmd(processDefinitionId, activityId);
	...
	@GET
	public List<ActivityDTO> findTaskDefinitionKeys(@QueryParam("processDefinitionId") String processDefinitionId) {
		FindTaskDefinitionsCmd cmd = new FindTaskDefinitionsCmd(processDefinitionId);

		return this.convertActivityDtoFromTaskDefinitions(processEngine.getManagementService().executeCommand(cmd));
	}
	...
	public List<ActivityDTO> convertActivityDtoFromTaskDefinitions(List<TaskDefinition> taskDefinitions) {
		List<ActivityDTO> activityDtos = new ArrayList<ActivityDTO>();

		for (TaskDefinition taskDefinition : taskDefinitions) {
			ActivityDTO activityDto = new ActivityDTO();
			activityDto.setId(taskDefinition.getKey());
			activityDto.setName(taskDefinition.getNameExpression().getExpressionText());
			activityDtos.add(activityDto);
		}
