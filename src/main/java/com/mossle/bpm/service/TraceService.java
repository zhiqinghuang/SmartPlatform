...
public class TraceService {
	...
	public List<NodeDTO> traceProcessInstance(String processInstanceId) {
		...
		BpmnModel bpmnModel = processEngine.getManagementService().executeCommand(getBpmnModelCmd);
		...
		for (Map.Entry<String, GraphicInfo> entry : graphicInfoMap.entrySet()) {
			...
			nodeDtos.add(this.convertNodeDto(graphicInfo, bpmnModel.getFlowElement(key), key, bpmnModel));
		}

		return nodeDtos;
	}

	public NodeDTO convertNodeDto(GraphicInfo graphicInfo, FlowElement flowElement, String id, BpmnModel bpmnModel) {
		...
		if (flowElement instanceof FlowNode) {
			...
			for (SequenceFlow sequenceFlow : flowNode.getOutgoingFlows()) {
				...
				for (GraphicInfo flowGraphicInfo : bpmnModel.getFlowLocationGraphicInfo(sequenceFlow.getId())) {
					List<Integer> position = new ArrayList<Integer>();
					position.add((int) flowGraphicInfo.getX() - ((int) graphicInfo.getWidth() / 2));
					position.add((int) flowGraphicInfo.getY() - ((int) graphicInfo.getHeight() / 2));
