...
public class UserAvatarService {
	private static Logger logger = LoggerFactory.getLogger(UserAvatarService.class);
	private AccountInfoManager accountInfoManager;
	private AccountAvatarManager accountAvatarManager;
	private StoreConnector storeConnector;

	public DataSource viewAvatar(Long accountId, int width, String tenantId) throws Exception {
		...
		if (storeDto == null) {
			storeDto = storeConnector.getStore("avatar", "default.jpg", tenantId);
		...
		StoreDTO resizeStoreDto = storeConnector.getStore("avatar", resizeKey, tenantId);

		if (resizeStoreDto == null) {
			ByteArrayOutputStream baos = new ByteArrayOutputStream();
			ImageUtils.zoomImage(originalStoreDto.getDataSource().getInputStream(), baos, width, width);
			logger.info("resizeKey : {}", resizeKey);
			resizeStoreDto = storeConnector.saveStore("avatar", resizeKey, new ByteArrayDataSource(storeDto.getDataSource().getName(), baos.toByteArray()), tenantId);
		}
