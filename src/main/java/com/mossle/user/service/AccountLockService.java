...
public class AccountLockService {
	private static Logger logger = LoggerFactory.getLogger(AccountLockService.class);
	...
	public void addLockLog(String username, String application, Date logDate) {
		...
		List<AccountLockLog> accountLockLogs = accountLockLogManager.find(logHql, application, username, calendar.getTime());
	...
	public void unlock(String username, String application) {
		...
		List<AccountLockLog> accountLockLogs = accountLockLogManager.find(logHql, application, username);
		accountLockLogManager.removeAll(accountLockLogs);

		String infoHql = "from AccountLockInfo where type=? and username=?";
		List<AccountLockInfo> accountLockInfos = accountLockInfoManager.find(infoHql, application, username);
	...
	public void doUnlock() throws Exception {
		...
		List<AccountLockInfo> accountLockInfos = accountLockInfoManager.find(hqlAccountLockInfo, new Date());
		...
		for (AccountLockInfo accountLockInfo : accountLockInfos) {
			...
			List<AccountLockLog> accountLockLogs = accountLockLogManager.find(hqlAccountLockLog, accountLockInfo.getType(), accountLockInfo.getUsername());
			accountLockLogManager.removeAll(accountLockLogs);
		}
		...
		List<AccountLockLog> accountLockLogs = accountLockLogManager.find(hqlAccountLockLog, calendar.getTime());
	...
	public void doClean() throws Exception {
		...
		List<AccountLockInfo> accountLockInfos = accountLockInfoManager.getAll();
