...
public class DatabaseUserRepoConnector implements UserRepoConnector {
	private static Logger logger = LoggerFactory.getLogger(DatabaseUserRepoConnector.class);
	...
	protected UserRepoDTO convertUserRepoDTO(Map<String, Object> map) {
		...
		userRepoDto.setId((map.get("id") == null) ? null : map.get("id").toString());
		userRepoDto.setCode((map.get("code") == null) ? null : map.get("code").toString());
		userRepoDto.setName((map.get("name") == null) ? null : map.get("name").toString());
