...
public class AccountStatusResource {
	private static Logger logger = LoggerFactory.getLogger(AccountStatusResource.class);
	private AccountStatusHelper accountStatusHelper;
	private AccountAliasConverter accountAliasConverter;

	@GET
	@Path("islocked")
	public BaseDTO isLocked(@QueryParam("username") String username, @QueryParam("application") String application) {
	...
	@GET
	@Path("accountstatus")
	public BaseDTO accountStatus(@QueryParam("username") String username, @QueryParam("application") String application) {
		BaseDTO baseDto = new BaseDTO();

		try {
			baseDto.setCode(200);
			baseDto.setData(accountStatusHelper.getAccountStatus(username, application));
		} catch (Exception ex) {
			logger.error(ex.getMessage(), ex);
			baseDto.setCode(500);
			baseDto.setMessage(ex.getMessage());
