...
public class AccountLogResource {
	private static Logger logger = LoggerFactory.getLogger(AccountLogResource.class);
	private AccountLogQueue accountLogQueue;

	@POST
	public BaseDTO log(@FormParam("username") String username, @FormParam("result") String result, @FormParam("reason") String reason, @FormParam("application") String application, @FormParam("logTime") String logTime, @FormParam("client") String client, @FormParam("server") String server, @FormParam("description") String description) {
	...
	public Date tryToParseTime(String text) {
		...
		try {
			Date date = new SimpleDateFormat("yyyy-MM-dd'T'HH:mm:ss.S").parse(text);
