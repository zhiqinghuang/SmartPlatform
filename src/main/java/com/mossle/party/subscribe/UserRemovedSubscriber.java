...
public class UserRemovedSubscriber implements Subscribable<String> {
	...
	public void handleMessage(String message) {
		...
		try {
			...
			Long entityId = this.getPartyEntityId(userDto.getId(), typeId, tenantId);
	...
	public Long getPartyEntityId(String reference, Long typeId, String tenantId) {
		try {
			return jdbcTemplate.queryForObject(selectPartyEntitySql, Long.class, reference, typeId, tenantId);
		} catch (Exception ex) {
			logger.error(ex.getMessage(), ex);
