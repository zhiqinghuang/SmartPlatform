...
public class PartyOrgConnector implements OrgConnector {
	private static Logger logger = LoggerFactory.getLogger(PartyOrgConnector.class);
	...
	public PartyEntity findUser(String userId) {
		...
		PartyEntity partyEntity = partyEntityManager.findUnique(hql, PartyConstants.TYPE_USER, userId);
	...
	public int getJobLevelByInitiatorAndPosition(String userId, String positionName) {
		...
		PartyEntity partyEntity = partyEntityManager.findUnique(hql, PartyConstants.TYPE_POSITION, positionName);
	...
	public String getSuperiorId(String userId) {
		...
		PartyEntity partyEntity = this.findUser(userId);
		logger.debug("party entity : {}, {}", partyEntity.getId(), partyEntity.getName());
	...
	public PartyEntity findSuperior(PartyEntity child) {
		...
		while (partyEntity != null) {
			logger.debug("partyEntity : {}, {}", partyEntity.getId(), partyEntity.getName());

			// 遍历上级部门的每个叶子
			for (PartyStruct partyStruct : partyEntity.getChildStructs()) {
				PartyEntity childPartyEntity = partyStruct.getChildEntity();
				logger.debug("child : {}, {}", childPartyEntity.getId(), childPartyEntity.getName());

				PartyEntity superior = null;

				// 如果叶子是岗位，并且这个职位是管理岗位
				if ((childPartyEntity.getPartyType().getType() == PartyConstants.TYPE_POSITION) && (partyStruct.getAdmin() == 1)) {
					// 就找这个岗位下面的管理者
					superior = this.findAdministrator(childPartyEntity);
				} else if ((childPartyEntity.getPartyType().getType() == PartyConstants.TYPE_USER) && (partyStruct.getAdmin() == 1)) {
	 ...
	 * 找到离parent最近的岗位下的人员.
	 */
	public List<String> findPositionUserIds(PartyEntity parent, String positionName) {
		...
		while (partyEntity != null) {
			...
			if (partyEntity.getPartyType().getType() == PartyConstants.TYPE_ORG) {
				for (PartyStruct partyStruct : partyEntity.getChildStructs()) {
					...
					// 遍历组织下级岗位
					if ((child.getPartyType().getType() == PartyConstants.TYPE_POSITION) && child.getName().equals(positionName)) {
			...
			} else if ((parent.getPartyType().getType() == PartyConstants.TYPE_POSITION) && parent.getName().equals(positionName)) {
	...
	public PartyEntity findUpperDepartment(PartyEntity child) {
		for (PartyStruct partyStruct : child.getParentStructs()) {
			PartyEntity parent = partyStruct.getParentEntity();
			logger.debug("parent : {}, child : {}", parent.getName(), child.getName());
			...
			if ((parent.getPartyType().getType() == PartyConstants.TYPE_ORG) && (!Integer.valueOf(1).equals(partyStruct.getAdmin()))) {
				logger.debug("upper department : {}, admin : [{}]", parent.getName(), partyStruct.getAdmin());
