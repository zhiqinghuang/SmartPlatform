	...
	public void invokeMethod(HttpServletRequest request, HttpServletResponse response, Object object, Method method, Map<String, String> parameters) {
		try {
			...
			for (int i = 0; i < annotationArray.length; i++) {
				...
				if (value != null) {
					if (parameterTypeArray[i] == String.class) {
						arguments.add(value);
					} else if ((parameterTypeArray[i] == Boolean.class) || (parameterTypeArray[i] == boolean.class)) {
						...
						throw new IllegalArgumentException("unsupport type : " + parameterTypeArray[i]);
					}
				} else {
					if (parameterTypeArray[i] == boolean.class) {
						...
						String message = "cannot process method argument, index is : " + i + ", name : " + name + ", value is null";
			...
			if (result instanceof String) {
				...
				IoUtils.copyStream((InputStream) result, response.getOutputStream());
			} else {
				response.setContentType(MediaType.APPLICATION_JSON);
				response.getOutputStream().write(jsonMapper.toJson(result).getBytes("UTF-8"));
			}
		} catch (Exception ex) {
