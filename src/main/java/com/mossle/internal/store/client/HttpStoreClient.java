...
public class HttpStoreClient implements StoreClient {
	...
	public StoreDTO saveStore(InputStream inputStream, String fileName, String contentType, String tenantId) throws Exception {
		...
		String content = new String(new Base64().encodeBase64(baos.toByteArray()));

		String queryString = "model=" + model + "&fileName=" + fileName + "&contentType=" + URLEncoder.encode(contentType, "utf-8") + "&content=" + URLEncoder.encode(content, "utf-8") + "&tenantId=" + URLEncoder.encode(tenantId, "utf-8");
	...
	public StoreDTO getStore(String key, String tenantId) throws Exception {
		String queryString = "model=" + model + "&key=" + key + "&tenantId=" + tenantId;
		...
		storeDto.setModel(model);
		storeDto.setKey(key);
		storeDto.setDataSource(new InputStreamDataSource(new ByteArrayInputStream(b2)));
