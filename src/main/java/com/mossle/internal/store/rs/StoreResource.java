...
public class StoreResource {
	...
	public BaseDTO getStore(@QueryParam("model") String model, @QueryParam("key") String key, @QueryParam("tenantId") String tenantId) {
		try {
			...
			String base64 = new String(new Base64().encodeBase64(baos.toByteArray()));
	...
	@Path("saveStore")
	@Produces(MediaType.APPLICATION_JSON)
	public BaseDTO saveStore(@FormParam("model") String model, @FormParam("content") String content, @FormParam("fileName") String fileName, @FormParam("contentType") String contentType, @QueryParam("tenantId") String tenantId) {
		try {
	...
	@GET
	@Path("view")
	public void view(@QueryParam("model") String model, @QueryParam("key") String key, HttpServletRequest request, HttpServletResponse response) throws Exception {
		String tenantId = tenantHolder.getTenantId();
		StoreDTO storeDto = storeConnector.getStore(model, key, tenantId);

		InputStream is = storeDto.getDataSource().getInputStream();
		ServletUtils.setFileDownloadHeader(request, response, storeDto.getDataSource().getName());
