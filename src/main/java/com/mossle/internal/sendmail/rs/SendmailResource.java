...
public class SendmailResource {
	private static Logger logger = LoggerFactory.getLogger(SendmailResource.class);
	private SendmailDataService sendmailDataService;
	private JsonMapper jsonMapper = new JsonMapper();

	@Path("send")
	@POST
	public BaseDTO send(@FormParam("to") String to, @FormParam("subject") String subject, @FormParam("content") String content, @FormParam("configCode") String configCode, @FormParam("tenantId") String tenantId) {
		...
		try {
			sendmailDataService.send(to, subject, content, configCode, tenantId);
			baseDto.setCode(200);
	...
	@Path("sendTemplate")
	@POST
	public BaseDTO sendTemplate(@FormParam("to") String to, @FormParam("data") String data, @FormParam("templateCode") String templateCode, @FormParam("configCode") String configCode, @FormParam("tenantId") String tenantId) {
		logger.debug("to : {}", to);
		...
		boolean templateExists = sendmailDataService.checkTemplateCodeExists(templateCode, tenantId);
		...
		boolean configExists = sendmailDataService.checkConfigCodeExists(configCode, tenantId);
		...
		try {
			sendmailDataService.sendTemplate(to, data, templateCode, configCode, tenantId);
			baseDto.setCode(200);
			logger.debug("success");
		} catch (Exception ex) {
			logger.debug("error");
