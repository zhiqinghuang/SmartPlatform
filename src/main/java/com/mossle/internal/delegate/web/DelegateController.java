...
public class DelegateController {
	...
	public String listMyDelegateInfos(Model model) {
		...
		List<DelegateInfo> delegateInfos = delegateInfoManager.findBy("assignee", userId);
	...
	public String prepareAutoDelegate(Model model) {
		...
		Page page = processConnector.findProcessDefinitions(tenantId, new Page(1, 100));
	...
	public String autoDelegate(@RequestParam(value = "startTime", required = false) Date startTime, @RequestParam(value = "endTime", required = false) Date endTime, @RequestParam("processDefinitionId") String processDefinitionId, @RequestParam("taskDefinitionKey") String taskDefinitionKey, @RequestParam("attorney") String attorney) throws Exception {
		String userId = currentUserHolder.getUserId();
		String tenantId = tenantHolder.getTenantId();
		...
		delegateService.addDelegateInfo(userId, attorney, startTime, endTime, processDefinitionId, taskDefinitionKey, tenantId);
	...
	public String listDelegateInfos(Model model) {
		String tenantId = tenantHolder.getTenantId();
		List<DelegateInfo> delegateInfos = delegateInfoManager.findBy("tenantId", tenantId);
		model.addAttribute("delegateInfos", delegateInfos);
	...
	public String listDelegateHistories(Model model) {
		String tenantId = tenantHolder.getTenantId();
		List<DelegateHistory> delegateHistories = delegateHistoryManager.findBy("tenantId", tenantId);
	...
	@Resource
	public void setDelegateHistoryManager(DelegateHistoryManager delegateHistoryManager) {
