...
public class WhitelistResource {
	...
	public BaseDTO getWhitelist(@QueryParam("code") String code, @QueryParam("tenantId") String tenantId) {
		BaseDTO baseDto = new BaseDTO();
		WhitelistType whitelistType = whitelistTypeManager.findUnique("from WhitelistType where code=? and tenantId=?", code, tenantId);
		...
		for (WhitelistApp whitelistApp : whitelistType.getWhitelistApps()) {
			...
			map.put("forceRelogin", Integer.valueOf(1).equals(whitelistApp.getForceRelogin()));
