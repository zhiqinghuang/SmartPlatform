...
public class JavamailService {
	...
	public Properties createSmtpProperties(JavamailConfig javamailConfig) {
		...
		props.setProperty("mail.transport.protocol", javamailConfig.getSendType());
		...
		if ("ssl".equals(javamailConfig.getSendSecure())) {
			...
			props.setProperty("mail.smtp.ssl.trust", javamailConfig.getSendHost());
	...
	public Properties createPop3Properties(JavamailConfig javamailConfig) {
		Properties props = new Properties();
		props.setProperty("mail.store.protocol", javamailConfig.getReceiveType());
		...
		if ("ssl".equals(javamailConfig.getReceiveSecure())) {
			props.setProperty("mail.pop3.ssl.enable", "true");
			props.setProperty("mail.pop3.ssl.trust", javamailConfig.getReceiveHost());
	...
	public void send(String from, String to, String subject, String content) throws MessagingException {
		JavamailConfig javamailConfig = javamailConfigManager.findUniqueBy("userId", from);
		this.send(to, subject, content, javamailConfig);
	}

	public void send(String to, String subject, String content, JavamailConfig javamailConfig) throws MessagingException {
		this.send(to, null, null, subject, content, javamailConfig);
	}

	public void send(String to, String cc, String bcc, String subject, String content, JavamailConfig javamailConfig) throws MessagingException {
	...
	public void receive(String userId) throws MessagingException, IOException {
		JavamailConfig javamailConfig = javamailConfigManager.findUniqueBy("userId", userId);
		this.receive(javamailConfig);
	}

	public void receive(JavamailConfig javamailConfig) throws MessagingException, IOException {
		this.receivePop3(javamailConfig);
	}

	public void receivePop3(JavamailConfig javamailConfig) throws MessagingException, IOException {
		// 准备连接服务器的会话信息
		...
		store.connect(javamailConfig.getUsername(), javamailConfig.getPassword());
		...
		for (Message message : messages) {
			...
			try {
				if (javamailMessageManager.findUniqueBy("messageId", mimeMessage.getMessageID()) != null) {
					continue;
				}
			} catch (Exception ex) {
	...
	@Resource
	public void setJavamailMessageManager(JavamailMessageManager javamailMessageManager) {
		this.javamailMessageManager = javamailMessageManager;
	}

	@Resource
	public void setJavamailConfigManager(JavamailConfigManager javamailConfigManager) {
