...
public class JavamailController {
	...
	public String index(@RequestParam(value = "id", required = false) Long id, Model model) throws Exception {
		...
		if (javamailConfig != null) {
			...
			List<JavamailMessage> javamailMessages = javamailMessageManager.find(hql, javamailConfig.getId());
			model.addAttribute("javamailMessages", javamailMessages);
		}

		if (id != null) {
			model.addAttribute("javamailMessage", javamailMessageManager.get(id));
		}
	...
	@RequestMapping("send")
	public String send(@RequestParam("receiver") String receiver, @RequestParam("subject") String subject, @RequestParam("content") String content) throws Exception {
		String userId = currentUserHolder.getUserId();
		JavamailConfig javamailConfig = javamailConfigManager.findUniqueBy("userId", userId);
		// this.javamailService.send(receiver, subject, content,
		// javamailConfig);
	...
	public String config(Model model) throws Exception {
		JavamailConfig javamailConfig = javamailConfigManager.findUniqueBy("userId", currentUserHolder.getUserId());
	...
	public String configSave(@ModelAttribute JavamailConfig javamailConfig, Model model) throws Exception {
		JavamailConfig dest = javamailConfigManager.findUniqueBy("userId", currentUserHolder.getUserId());
